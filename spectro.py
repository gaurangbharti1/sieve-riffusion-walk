...
def wav_bytes_from_spectrogram_image(
    image: Image.Image) -> T.Tuple[io.BytesIO, float]:
  """
  ...
  Sxx = spectrogram_from_image(image,
  ...
  # Derived parameters
  num_samples = int(
    image.width / float(bins_per_image) * clip_duration_ms) * sample_rate
...
def spectrogram_from_image(image: Image.Image,
                           max_volume: float = 50,
                           power_for_image: float = 0.25) -> np.ndarray:
  """
...
def spectrogram_from_waveform(
  ...
  waveform_tensor = torch.from_numpy(waveform.astype(np.float32)).reshape(
    1, -1)
